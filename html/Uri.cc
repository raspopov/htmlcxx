...
Uri::Uri(const string &uri_str) throw (Exception)
: mScheme(), mUser(), mPassword(), mHostname(), mPath(), mQuery(), mFragment(), mPort(0), mExistsFragment(false), mExistsQuery(false)
{
	init(uri_str);
}

void Uri::init(const string &uri_str) throw (Exception)
{
...
static uint www_prefix_offset(const char *prefix) {
	uint len = strlen(prefix);
	if(strncmp("www", prefix, 3) == 0) {
		if(len > 3 && prefix[3] == '.') {
			return 4;
		}
		if(len > 4 && isdigit(prefix[3]) && prefix[4] == '.') {
			return 5;
		}
	}
	return 0;
...
string Uri::unparse(int flags ) const
{
	...
	if(!mHostname.empty()) { 
		...
		if(flags & Uri::REMOVE_WWW_PREFIX && mHostname.length() > 3) {
			offset = www_prefix_offset(mHostname.c_str());
		}
